    ...
    <div style={{display:"flex", flexDirection:"column"}}>
      <div className="form-row">
        ...
        <Typography sx={{ fontSize: 17, width:"75%", textAlign:"center" }}> What features do you want in the smartphone? </Typography>
          <FormGroup sx={{display:"flex", flexDirection:"row",  width:"75%", alignItems:"center", justifyContent:"space-between"}}>
            <FormControlLabel sx={{ flexbasis:"25%" }} value="camera" id="2" onChange={() => handleCheckboxChange(2)} control={<Checkbox sx={{ '& .MuiSvgIcon-root': { fontSize: 25 } }} />} label={<Typography sx={{ fontSize: 15 }}>
              Good camera
            </Typography>} />
            <FormControlLabel sx={{flexbasis:"25%" }}value="performance" id="1" checked={goodPerformance} onChange={() => { handleCheckboxChange(1); handleGoodPerformanceChange() }} control={<Checkbox sx={{ '& .MuiSvgIcon-root': { fontSize: 25 } }} />} label={<Typography sx={{ fontSize: 15 }}>
              Good performance
            </Typography>} />
            <FormControlLabel sx={{ flexbasis:"25%" }}value="battery" id="3" onChange={() => handleCheckboxChange(3)} control={<Checkbox sx={{ '& .MuiSvgIcon-root': { fontSize: 25 } }} />} label={<Typography sx={{ fontSize: 15 }}>
              Good battery
            </Typography>} />
            <FormControlLabel sx={{ flexbasis:"25%" }}value="browsing" id="4" checked={goodForBrowsing} onChange={() => { handleCheckboxChange(4); handleGoodForBrowsingChange() }} control={<Checkbox sx={{ '& .MuiSvgIcon-root': { fontSize: 25 } }} />} label={<Typography sx={{ fontSize: 15 }}>
              Good for browsing
            </Typography>}/>
          </FormGroup>
        </div>
