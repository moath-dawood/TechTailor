...
import PCDialog from './COMPDialog';
...
<Dialog open={openDialog} onClose={handleCloseDialog}>
        <DialogTitle>{compInfo?.name}</DialogTitle>
        <DialogContent>
          <DialogContentText>
        ...
        <MTable sx={{ minWidth: 500 }} aria-label="simple table">
          ...
          <TableBody>
            ...
            <TableRow  onClick={() => handleCellClick(PC?.gpu)}>
              ...
              <TableCell sx={{display:"flex",fontSize:"18px", alignItems:"center"}}><img className="comp-image" src={PC?.gpu?.external_image || PC?.gpu?.image} alt={PC?.name}/> {PC?.gpu?.name}</TableCell>
